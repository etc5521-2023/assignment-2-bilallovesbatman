...
ggplot(v, aes(
  ...
  geom_point() + 
  geom_abline(intercept = 0, slope = 1, color = "steelblue", linetype = "solid") +
  labs(x = "Viewport Width (pixels)", y = "Viewport Height (pixels)", size = "Count")
```
...
v <- v |>
  mutate(aspect_ratio = paste(Width / gcd(Width,Height), ":", Height / gcd(Width, Height)))
```
